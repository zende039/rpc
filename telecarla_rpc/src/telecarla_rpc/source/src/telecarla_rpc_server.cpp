...
TeleCarlaRpcServer::TeleCarlaRpcServer(ros::NodeHandle& nh, ros::NodeHandle& pnh)
    : server_("0.0.0.0", pnh.param("rpc_port", 2002)),  // Bind to all available interfaces
      egoVehicleStatus_(boost::make_shared<carla_msgs::CarlaEgoVehicleStatus>())
{
    ROS_INFO_STREAM("Opening RPC Server at IP 0.0.0.0 and port " << pnh.param("rpc_port", 2002));
